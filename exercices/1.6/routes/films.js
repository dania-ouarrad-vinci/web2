...
router.delete("/:id", function (req, res) {
  ...
  const foundIndex = FILM.findIndex((film) => film.id == req.params.id); // la méthode findIndex() recherche l'index de l'élément du tableau dont la propriété id = req.params.id, . permet de trouver l'index de l'élément à supprimer.

  if (foundIndex < 0) return res.sendStatus(404);

  const itemsRemovedFromFILM = FILM.splice(foundIndex, 1); // la méthode splice() supprime un (1) seul élément du tableau  à partir de l'index foundIndex,  La méthode splice() renvoie un tableau contenant les éléments supprimés, dans ce cas, un tableau contenant un seul élément. car on supp 1 seul
...
router.patch("/:id", function (req, res) {
  ...
  const foundIndex = FILM.findIndex((film) => film.id == req.params.id); // FILM.findIndex() recherche l'index de l'élément du tableau FILM dont la propriété id correspond à la valeur de req.params.id.

  if (foundIndex < 0) return res.sendStatus(404); // la fonction findIndex renvoie -1 si elle ne trouve pas de fim

  const filmPriorToChange = FILM[foundIndex]; // on recup le film a mettre à jour
  ...
  FILM[foundIndex] = updateFilm; // L'élément du tableau FILM à l'index foundIndex est mis à jour avec updateFilm.
  res.json(updateFilm);
});
