...
router.put('/:id', (req, res) => {
  ...
  const id = Number(req.params.id); // on extrait l'id à partir de req.params.id. Cet id est utilisé pour identifier l'élément du tableau FILM à mettre à jour ou à créer.
  const films = parse(jsonDbPath, FILM);
  const indexOfFilmFound = films.findIndex((film) => film.id === id); // Cela permet de trouver l'index de l'élément à mettre à jour.
  ...
  const filmPriorToChange = films[indexOfFilmFound]; // le code extrait l'élément à mettre à jour (filmPriorToChange) à partir du tableau FILM à cet index.
  const objectContainingPropertiesToBeUpdated = req.body; // les propriétés à mettre à jour sont stockées dans objectContainingPropertiesToBeUpdated à partir de req.body.
  const updatedFilm = {
    // nouvel objet updatedFilm est créé en fusionnant les propriétés de filmPriorToChange et objectContainingPropertiesToBeUpdated à l'aide de l'opérateur de décomposition (...). Cela crée un nouvel objet qui contient toutes les propriétés mises à jour.
    ...filmPriorToChange,
    ...objectContainingPropertiesToBeUpdated,
  };

  films[indexOfFilmFound] = updatedFilm; // L'élément du tableau FILM à l'index indexOfFilmFound est mis à jour avec updatedFilm
  serialize(jsonDbPath, films);
